...
def invoke_predict(request: dict):
    return predict_future(request)


def predict_future(request: dict):
    n_days = request.get("n_days", 10)
    seq_length = request.get("seq_length", 5)
    hidden_size = request.get("hidden_size", 512)
    n_layers = request.get("n_layers", 2)
    dropout = request.get("dropout", 0.5)
    num_epochs = request.get("num_epochs", 300)
